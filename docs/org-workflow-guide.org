#+title: Org-Mode Hybrid Workflow Guide
#+author: eemacs
#+startup: content indent

* Directory Structure

Your org system spans two layers: a central =~/org/= hub and per-project org files.

** Central Hub

#+begin_example
~/org/
├── inbox.org          <- everything lands here first (GTD inbox)
├── projects.org       <- master project list with links
├── journal.org        <- daily log (datetree)
├── archive/           <- completed/killed items go here
└── roam/              <- org-roam knowledge graph (interlinked notes)
#+end_example

** Per-Project (distributed)

#+begin_example
~/projects/my-dsp-project/
├── src/
├── TODO.org           <- project tasks (auto-added to agenda)
└── notes/             <- project notes (auto-indexed by org-roam)
    ├── architecture.org
    └── experiments.org

~/projects/ml-research/
├── TODO.org
└── notes/
    └── model-comparison.org
#+end_example

Some projects keep a single =notes.org= inside =notes/=.  This works just as well:

#+begin_example
~/projects/TCell-research/
├── simulation/
├── TODO.org           <- project tasks
└── notes/
    └── notes.org      <- all project notes in one file (roam node)
#+end_example

The agenda aggregates TODOs from both layers. Org-roam indexes notes from both layers into one unified knowledge graph.

* First-Time Setup

The central hub directory is not auto-created.  Before your first capture,
make sure =~/org/= exists with the two subdirectories that are referenced
as directories (not files):

#+begin_src sh
mkdir -p ~/org/archive ~/org/roam
#+end_src

The central files (=inbox.org=, =projects.org=, =journal.org=) are
created automatically by =org-capture= the first time their template
fires --- you do not need to pre-create them.
=org-roam/= is also auto-created by org-roam itself on load, but
creating it up front avoids a one-time startup warning.

** The Central Project List (=projects.org=)

=projects.org= is a first-class agenda file --- it is loaded into
=org-agenda-files= automatically (when it exists) and indexed for refile
and search.  Treat it as your master index, written freely:

#+begin_example
,#+title: Projects Master List
,#+filetags: :index:

,* DSP signal-processing library                                    :research:
  - Code:  [[file:~/projects/my-dsp-project/]]
  - Tasks: [[file:~/projects/my-dsp-project/TODO.org]]
  - Notes: [[file:~/projects/my-dsp-project/notes/]]

  Free-form status, decisions, and links to roam nodes go here.

,* TCell research                                                   :research:
  - Tasks: [[file:~/projects/TCell-research/TODO.org]]
  - Notes: [[file:~/projects/TCell-research/notes/notes.org]]
#+end_example

Each top-level heading inside =projects.org= is a project --- the
=:index:= filetag on the file already marks the whole file as a project
list, so no per-heading keyword is needed.  Giving the file an =:ID:=
(=M-x org-id-get-create=) also makes it a roam node, so =C-c n f= finds
it and project notes can link back to it.  Archive finished projects
with =C-c C-x C-a=.

* Capturing (The Entry Point for Everything)

Press =C-c x= from anywhere in Emacs. Pick a template:

** Central Captures

*** =t= --- Todo

Something you need to do. Goes to central inbox.  No auto-clock --- if
you want to time the task, start the clock manually with =C-c C-x C-i=.

#+begin_example
,* TODO Buy new oscilloscope
  [2026-02-27 Thu 14:30]
#+end_example

*** =j= --- Journal

Daily log entry. Goes into =journal.org= under today's date tree. Use this for daily standups, research log, what you worked on.

#+begin_example
,* 2026
,** 2026-02 February
,*** 2026-02-27 Thursday
,**** Debugged the FFT implementation
     Turns out the window function was wrong...
#+end_example

For research notes that aren't tasks, use =C-c n c= (=org-roam-capture=)
rather than =C-c x=.  Org-capture is for items that need an active state
or a date; org-roam-capture is for knowledge nodes.

** Project-Local Captures

This files directly into the current project's =TODO.org=, so tasks stay with the code.

| Key  | Template     | Destination          |
|------+--------------+----------------------|
| =p=  | Project todo | =<project>/TODO.org= |

The "current project" is detected automatically from the file you're editing (via =project.el=). If you're editing =~/projects/my-dsp-project/src/fft.c= and press =C-c x p=, the TODO goes into =~/projects/my-dsp-project/TODO.org=.

*Caveat:* if you invoke a =p= capture from a buffer that is not inside a
project (e.g. =*scratch*=, =*Messages*=, dired), the project root falls
back to =default-directory= and the TODO will be filed there.  Make sure
you are visiting a file inside a project before using =C-c x p ...=.

* Multi-Project Management

** Registering a Project

To make a project's TODOs appear in your agenda and its notes searchable in org-roam:

#+begin_example
C-c P r  ~/projects/my-dsp-project RET
#+end_example

This does four things:
1. Adds =TODO.org= to your agenda (creates it if missing)
2. Creates a =notes/= directory (if missing) for org-roam notes
3. Syncs the org-roam database so notes are immediately searchable
4. Persists the registration across Emacs restarts

** Unregistering a Project

#+begin_example
C-c P u
#+end_example

Pick from a list of registered projects. This removes it from the agenda and org-roam indexing. It does NOT delete any files.

** Listing Projects

#+begin_example
C-c P l
#+end_example

Shows all registered project directories.

** Example: Setting Up a New Research Project

#+begin_example
;; 1. Register the project
C-c P r  ~/projects/new-research RET

;; 2. Open the auto-created TODO.org
C-x C-f ~/projects/new-research/TODO.org RET

;; 3. Set up your project structure
,* New Research Project :research:
,** TODO Literature review
,** TODO Set up experiment framework
,** TODO Investigate method X

;; 4. Create a roam note in the project
;;    (from any file in the project)
C-c n c  ->  creates a note in ~/projects/new-research/notes/
#+end_example

** Example: Integrating an Existing Project That Has =notes.org=

If you already have a project with a =notes.org= file (e.g. inside =notes/=), you can make it part of the org-roam knowledge graph:

#+begin_example
;; 1. Register the project
C-c P r  ~/projects/TCell-research RET

;; 2. Open the existing notes file
C-x C-f ~/projects/TCell-research/notes/notes.org RET

;; 3. Turn it into an org-roam node:
;;    - Place point at the very top of the file
;;    - M-x org-id-get-create  (adds an :ID: property)
;;    - Add a #+title: line if there isn't one

;; The top of the file should now look like:
:PROPERTIES:
:ID:       <auto-generated-uuid>
:END:
#+title: TCell Research Notes

,* Description
...existing content...
#+end_example

After this, =C-c n f= will find the file by title from anywhere, =C-c n i= can link to it, and it appears in the org-roam graph.

If =notes.org= is at the project root instead of inside =notes/=, move it:

#+begin_example
mkdir -p ~/projects/my-project/notes
mv ~/projects/my-project/notes.org ~/projects/my-project/notes/
#+end_example

* Processing the Inbox (Daily/Weekly Ritual)

Open =inbox.org= and go through each item. For each one, decide:

| Decision                | Action                           | Keys                          |
|-------------------------+----------------------------------+-------------------------------|
| Do it now (< 2 min)    | Do it, mark DONE                 | =C-c C-t d=                  |
| Schedule it             | Add a date, refile to agenda     | =C-c C-s= then =C-c C-w=    |
| It's part of a project  | Refile to project's TODO.org     | =C-c C-w= -> pick project    |
| It's reference material | Create org-roam node             | =C-c n f= -> new node        |
| Trash it                | Kill it                          | =C-c C-t k= (KILL)           |

Note: registered project =TODO.org= files appear as refile targets, so you can refile directly from inbox to any project.

* TODO States --- What Each One Means

The states form a workflow. Cycle with =C-c C-t= then press the shortcut key:

#+begin_example
TODO  ->  you need to do this
STRT  ->  you're actively working on this right now
WAIT  ->  blocked on someone/something external
---------
DONE  ->  completed
KILL  ->  cancelled / no longer relevant
#+end_example

For recurring tasks, use a =TODO= with a date repeater
(=SCHEDULED: <2026-05-22 Fri +1w>=) instead of a separate keyword.
For long-running pauses you control yourself, leave the state as =TODO=
or use =WAIT= --- the distinction between "blocked externally" and
"paused by me" rarely matters in the agenda.

** List-item checkboxes (inside a heading body)

These are not heading-level TODO states --- they're checkbox items in a
list, used to break a task into sub-items:

#+begin_example
,* TODO Prepare grant submission
  - [ ] Draft abstract
  - [-] Methods section
  - [X] Budget table
#+end_example

** Tags

A small canonical tag alist is preconfigured (selectable from =C-c C-q=):

| Group         | Tags                                       |
|---------------+--------------------------------------------|
| Context       | =@home= (h), =@work= (w), =@lab= (L)       |
| Kind          | =research= (r), =paper= (p), =experiment= (x) |
| Topic         | =emacs= (e), =linux= (l), =programming= (P) |
| Priority flag | =IMPORTANT= (!), =URGENT= (u)              |

Free-form tags are allowed too --- anything you type as
=:my-tag:other-tag:= is preserved and is searchable via =C-c a m=.  The
canonical alist just gives you single-key selection in the tag prompt.

** Example Project Structure

#+begin_example
,* Signal processing library :research:programming:
,** DONE Set up repository
,** STRT Implement FFT module
,*** TODO Write unit tests for edge cases
,*** DONE Basic radix-2 implementation
,** WAIT Get benchmark data from lab
,** TODO Investigate GPU acceleration
#+end_example

* Agenda --- Navigating Your Day

Press =C-c a= then:

| Key | View                                                                |
|-----+---------------------------------------------------------------------|
| =d= | *Today --- daily ritual* (scheduled + STRT + WAIT + inbox triage)   |
| =a= | Day agenda --- all scheduled items from central + project files     |
| =t= | All TODOs across ALL registered projects and central org            |
| =m= | Match by tag (e.g., =research+URGENT=)                              |
| =s= | Search full text across everything                                  |

=C-c a d= is the morning ritual view.  It shows four blocks: today's
scheduled and deadlined items, everything in =STRT= state, everything
in =WAIT= state, and every =TODO= still sitting in =inbox.org= (the
=Inbox to triage= block uses =CATEGORY="inbox"= which Org derives
automatically from the filename).  If a block grows long, that's the
signal: too many things in progress, follow-ups stalling, or inbox
backlog accumulating.

The agenda shows your day with the Unicode time grid:

#+begin_example
Day-agenda (W09):
Thursday  27 February 2026
  8:00 ┄┄┄┄┄ ┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄
 10:00 ┄┄┄┄┄ ┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄
  agenda:         10:00 Lab meeting
 12:00 ┄┄┄┄┄ ┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄
⭠ now ─────────────────────────
 14:00 ┄┄┄┄┄ ┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄
  my-dsp-project: Deadline: Write results section
  ml-research:    TODO Review paper draft
#+end_example

Tasks from different projects are clearly labeled with their source file.

** In Agenda View

- =K= --- start a pomodoro on the task at point
- =t= --- cycle TODO state
- =s= --- schedule
- =d= --- set deadline
- =r= --- refresh

* Org-Roam --- Unified Knowledge Graph

Org-roam indexes notes from *both* your central =~/org/roam/= and every registered project's =notes/= directory. All notes are part of one interconnected graph.

| Keys       | Action                                                  |
|------------+---------------------------------------------------------|
| =C-c n f=  | Find or create a node (searches all indexed dirs)       |
| =C-c n i=  | Insert a link to another node while writing             |
| =C-c n l=  | Toggle backlinks buffer                                 |
| =C-c n c=  | Capture a new roam note                                 |
| =C-c n j=  | Capture today's daily roam note                         |
| =C-c n g=  | View the graph                                          |
| =C-c n u=  | Interactive graph in browser (org-roam-ui)              |

=C-c n j= uses =org-roam-dailies=; daily entries land in
=~/org/roam/daily/YYYY-MM-DD.org= by default.  These are full roam nodes,
so they participate in backlinks and graph view like any other node.

** Cross-Project Linking

The power of a unified graph: notes in one project can link to notes in another.

#+begin_example
# ~/projects/my-dsp-project/notes/fft-optimization.org
#+title: FFT Optimization Approaches

See [[id:abc123][CUDA Kernel Patterns]] from the GPU project
for parallel implementation ideas.

The [[id:def456][Cooley-Tukey Algorithm]] note in the central
roam has the mathematical foundation.
#+end_example

** Research Workflow Example

1. =C-c n f= -> "Kalman Filter" -> creates node (in central roam or project notes)
2. While writing, =C-c n i= -> "Bayesian" -> links to another node (could be in any project)
3. =C-c n l= -> see all backlinks across all projects
4. =C-c n u= -> visual graph showing connections between projects

* Additional Features

** Pomodoro (Focused Work Sessions)

- =C-c C-x m= --- start 25-min pomodoro on current task
- =K= in agenda view --- start pomodoro on task at point
- Modeline shows countdown; 5-min break after each, longer break after 4

** Bibliography (Citations)

- =C-c C-x @= --- insert citation from =.bib= file
- Set =#+bibliography: ~/org/references.bib= in file header
- Exports to =\cite{}= with biblatex in LaTeX output

** Exporting

| Format              | Keys              |
|---------------------+-------------------|
| PDF (LaTeX)         | =C-c C-e l p=     |
| Beamer slides       | =C-c C-e l P=     |
| HTML                | =C-c C-e h h=     |
| GitHub Markdown     | =C-c C-e g g=     |
| ODT (LibreOffice)   | =C-c C-e o o=     |

Exports run in the background (async) so Emacs stays responsive.

* Advanced Topics

** Archiving

When tasks are completed and no longer relevant to the active agenda:

- =C-c C-x C-a= --- archive the entry at point
- Archived items move to an =archive/= directory *relative to the file
  they came from*, into =<filename>_archive=, and are removed from the
  active agenda but preserved for reference.

Concretely, with =org-archive-location= set to =archive/%s_archive::=:

| Source file                              | Archive destination                                |
|------------------------------------------+----------------------------------------------------|
| =~/org/inbox.org=                        | =~/org/archive/inbox.org_archive=                  |
| =~/projects/my-dsp-project/TODO.org=     | =~/projects/my-dsp-project/archive/TODO.org_archive= |

So each project carries its own =archive/= folder next to its
=TODO.org=.  This keeps project history with the code rather than in a
single global pile.

** Backup and Sync

=~/org/= grows quickly into a high-value, hard-to-replace dataset.  Pick
*one* of these:

- *Private git repo.*  Init =~/org/= as a repo on a private remote.
  Recommended =.gitignore=:
  #+begin_example
  roam/.org-roam.db
  roam/.org-roam.db-shm
  roam/.org-roam.db-wal
  *~
  \#*\#
  .#*
  #+end_example
  The roam database is a cache and rebuilds via =M-x org-roam-db-sync=.

- *Syncthing / Dropbox / iCloud Drive.*  Live multi-device merge without
  history.  Good if you edit from a phone or tablet.

- *Both.*  Syncthing for live sync between machines, git for history and
  off-site backup.  Run =git add -A && git commit -m "snapshot"= from a
  cron/systemd timer if you want zero-friction history.

For sensitive subtrees inside otherwise-public files, tag them =:crypt:=
and configure =org-crypt= --- safer than encrypting the whole repo.

** Per-Project .dir-locals.el

Project =notes/= directories are indexed by org-roam automatically once the project is registered --- no extra configuration needed.  For other project-specific org settings (e.g. a local bibliography), create a =.dir-locals.el= in the project root:

#+begin_src emacs-lisp
;;; .dir-locals.el
((org-mode . ((citar-bibliography . ("~/projects/my-dsp-project/references.bib")))))
#+end_src

* Daily Routine Summary

| When                | What                              | Keys                         |
|---------------------+-----------------------------------+------------------------------|
| *Morning*           | Open dashboard                    | =C-c d=                     |
| *Morning*           | Daily ritual view                 | =C-c a d=                   |
| *Throughout day*    | Capture anything                  | =C-c x= -> pick template    |
| *Working on project* | Capture to that project           | =C-c x p=                   |
| *Starting work*     | Start pomodoro                    | =C-c C-x m=                 |
| *While researching* | Create/link roam notes            | =C-c n f=, =C-c n i=        |
| *End of day*        | Journal entry                     | =C-c x j=                   |
| *Weekly*            | Process inbox, refile to projects | =inbox.org=, =C-c C-w=      |
| *Weekly*            | Review all projects               | open =projects.org=         |
| *New project*       | Register it                       | =C-c P r=                   |

* Quick Reference --- All Commands

** Global

| Keys       | Command               |
|------------+-----------------------|
| =C-c d=    | Open dashboard        |
| =C-c a=    | Open agenda           |
| =C-c x=    | Capture               |
| =C-c n f=  | Find/create roam node |
| =C-c n i=  | Insert roam link      |
| =C-c n l=  | Toggle backlinks      |
| =C-c n u=  | Roam graph UI         |
| =C-c n j=  | Daily roam note       |

** In Org Buffers

| Keys         | Command            |
|--------------+--------------------|
| =C-c C-t=    | Cycle TODO state  |
| =C-c C-s=    | Schedule          |
| =C-c C-d=    | Deadline          |
| =C-c C-w=    | Refile            |
| =C-c C-x C-a= | Archive entry   |
| =C-c l=      | Store link        |
| =C-c C-x m=  | Start pomodoro   |
| =C-c C-e=    | Export dispatcher |
| =M-h/l/j/k=  | Org meta movement |

** Project Management

| Keys       | Command                          | Action                      |
|------------+----------------------------------+-----------------------------|
| =C-c P r=  | =ee-org-register-project=       | Add project to agenda+roam  |
| =C-c P u=  | =ee-org-unregister-project=     | Remove project              |
| =C-c P l=  | =ee-org-list-projects=          | Show registered projects    |
